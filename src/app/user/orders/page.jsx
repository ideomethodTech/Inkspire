      ...
      <section className="rounded-lg bg-white p-8 shadow-sm">
        ...
        <div className="mb-8 flex items-center justify-between border-b border-neutral-200 pb-4">
          <div className="flex items-center gap-2">
            <ShoppingBag size={18} className="text-[#6D6D6D]" />
            <div>
                      ...
                      {isCurrent && (
                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap">
                          <div className="relative">
                            <div className="absolute -bottom-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-[#E11B1B]" />
                            <div className="rounded bg-[#E11B1B] px-2 py-1">
                    ...
                    {idx < ORDER.steps.length - 1 && (
                      <div
                        className={`absolute left-[12px] h-[2px] w-full ${
                          isCompleted ? "bg-[#FFA500]" : "bg-neutral-300"
                        }`}
                      />
                    )}
                  </div>
        ...
        <div className="space-y-6">
          {ORDER.items.map((item) => (
            <div key={item.id} className="flex items-center gap-4">
              <div className="relative h-28 w-24 overflow-hidden rounded-md bg-neutral-100">
                <Image
                  src={item.image}
                  alt={item.title}
                  fill
        ...
        <div className="mt-8 border-t border-neutral-200 pt-6">
          ...
          <Button className="w-full bg-black text-white hover:bg-black uppercase tracking-[0.08em]">
            Details
          </Button>
        </div>
      </section>
